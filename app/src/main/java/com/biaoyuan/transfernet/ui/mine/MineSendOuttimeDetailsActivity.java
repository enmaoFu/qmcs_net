...
public class MineSendOuttimeDetailsActivity extends BaseAty{
    ...
    public void initData() {
        ...
        /*qjDateNew = bundle.getString("qjDateNew");
        yqDateNew = bundle.getString("yqDateNew");
        cgDateNew = bundle.getString("cgDateNew");*/
        orderNo = bundle.getString("orderNo");

        initToolbar(mToolbar,"发出超时");
        codeTextView.setText("快件码：" + code);
        /*dateTextView.setText(qjDateNew);
        yqDate.setText(yqDateNew);
        cgDate.setText(cgDateNew);*/
    }
    ...
    public void onSuccess(String result, Call<ResponseBody> call, Response<ResponseBody> response, int what) {
        ...
        switch (what){
            case 1:
                ...
                orderWeightSize.setText(getSize + "cm   " + getWeight + "kg");
                ...
                dateTextView.setText("取件时间：" + getTakeDate);
                /**
                ...
                //获取取件时间后12小时
                long getDeliveryUpdateTime = Long.parseLong(AppJsonUtil.getString(AppJsonUtil.getString(result,"data"),"order_required_time")) + 60 * 60 * 12 * 1000;
                String yqDateNew = "要求时间：" + DateTool.timesToStrTime(getDeliveryUpdateTime + "", "yyyy.MM.dd HH:mm");
                yqDate.setText(yqDateNew);
                 ...
                 */
                //发出时间
                long recordTimel = Long.parseLong(AppJsonUtil.getString(AppJsonUtil.getString(result,"data"),"excep_record_time"));
                if(recordTimel > getDeliveryUpdateTime){
                    try{
                        SimpleDateFormat simpleFormat = new SimpleDateFormat("yyyy-MM-dd hh:mm");
                        String fromDate = simpleFormat.format(recordTimel);
                        String toDate = simpleFormat.format(getDeliveryUpdateTime);
                        long from = simpleFormat.parse(fromDate).getTime();
                        long to = simpleFormat.parse(toDate).getTime();
                        int hours = (int) ((from - to)/(1000 * 60 * 60));
                        cgDate.setText("(超时" + hours + "小时)");
                    }catch (Exception e){
                        e.printStackTrace();
                    }
                }else{
                    cgDate.setText("(超时0.00小时)");
                }
